...
| P21 | Prepared installation and backup/restore verified. The 43,618,304-byte pre-Codex-capture snapshot now has a full scratch restore proof. The later authorized one-task live capture is recorded separately; no live schema migration in final integration | Back up the live database with SQLite's backup mechanism; prove restoration using a copy. Install the chosen local update only after migration/recovery checks. Record the installed revision and store version; distinguish v2.4 installation from v2.5 development. |
